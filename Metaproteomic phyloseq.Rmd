...
Note that human proteins are already filtered out, but a protein may pass the filter if it has an identification made to multiple species (e.g. human *and* pig).  On inspection, these seem to be things we'd want to remove, like histones and ubiquitin.

```{r}
proteins.df %>% 
     filter(grepl('Human_*', Accession))
```
Check also for presence of microbiota proteins:

```{r}
proteins.df %>% 
     filter(grepl('Microbiota_*', Accession)) 
```
...
#             paste0(plotID, '_Master protein breakdown, V2 analysis.pdf')),
#        height = 4, width = 4)
```
...
I think the best thing to do here would be to group them and re-label with the last common ancestor.  However, the total number of PSMs here is 524, which is ~1.3% of the overall dataset, so-- skipping this for now.  Can return to and refine. 
...
add.rows <-
     here('data', 'processed', 
          ...
          # '20210727_Internal taxonomy table nodes.csv') %>% # Analysis v3-- all (erroneously)
...
```{r}
# How many taxa do I expect?
union(taxa_names(ps.dfc), 
      taxa_names(ps.908)) %>% 
     unique() %>% 
     length()
```
