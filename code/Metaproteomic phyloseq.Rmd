...
This is pre-filtered by Ali to only proteins with >=1 unique peptide.
...
fs <-
     here('data', 
          'raw',
          '3_DB2.1_5FDR_1UniquePeptide_Weight Loss') %>%
...
This is *not* pre-filtered by Ali to only proteins with >=1 unique peptide.  Will need to do this filtering here.
...
names(dfs) <- 
     basename(fs) %>% 
     gsub(pattern = '^HH\\d_SubjectID', replacement = '') %>% 
     gsub(pattern = '_(Monday|Tuesday|Wednesday|Thursday|Friday).*$', 
          replacement = '')
