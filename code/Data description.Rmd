...
protein.plant.taxa <- 
     ...
     data.frame(old = .) %>% 
     left_join(name.sync, by = c('old' = 'mp_taxa')) %>% 
     select(old, `taxon index`) %>% 
     distinct() %>% 
     pull(`taxon index`)
...
menu.plant.taxa <- 
     ...
     data.frame(old = .) %>% 
     left_join(name.sync, by = c('old' = 'menu_taxa')) %>% 
     select(old, `taxon index`) %>% 
     distinct() %>% 
     pull(`taxon index`)
...
```{r}
# Thinking that A here will be overview graphic
