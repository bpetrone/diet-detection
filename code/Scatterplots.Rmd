...
```{r eval = FALSE}
# Rename "all" (common_name) to grouped taxon (name)
pred.mb.all <- rename(pred.mb.all, name = common_name)
pred.mp.all <- rename(pred.mp.all, name = common_name)
```

```{r eval = FALSE}
# Annotate each dataset prior to joining
pred.mb.all$level <- 'all'
pred.mb.species$level <- 'species'
pred.mb.genus$level <- 'genus'
pred.mb.family$level <- 'family'

pred.mp.all$level <- 'all'
...
rm(pred.mb.all, pred.mb.species, pred.mb.genus, pred.mb.family)
...
rm(pred.mp.all, pred.mp.species, pred.mp.genus, pred.mp.family)
```


```{r eval = FALSE}
# Annotate detection, then make final join
...
pred <- 
     pivot_wider(pred, names_from = prediction, values_from = n) %>% 
     # Group and summarize across the two studies
     select(-study) %>%
     group_by(kingdom, common_name, marker) %>% 
     summarise(across(c(starts_with('True '),
                        starts_with('False')),
                      sum))
...
```
```{r eval = FALSE}
# Factor levels for plots
pred.summary$level <- factor(pred.summary$level,
                             levels = c('all',
                                        'species',
                                        'genus',
                                        'family'))
...
```{r}
ggplot(false.rates, aes(x = specificity_DNA, y = specificity_1PUP)) +
     geom_point(alpha = 0.5) +
     coord_equal()
```
