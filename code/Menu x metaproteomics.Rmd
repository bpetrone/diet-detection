...
tax_table(ps.protein) <- 
     ...
     right_join(data.frame(ps.protein@tax_table@.Data),
               by = c('mp_taxa' = 'name')) %>%
     distinct() %>% 
     select(superkingdom:species, mp_to_menu, mp_taxa) %>% 
     column_to_rownames(var = 'mp_taxa') %>% 
     as.matrix()

ps.protein.id <- tax_glom(ps.protein, taxrank = 'mp_to_menu')
...
tax_table(ps.menu.12) <- 
     ...
     right_join(data.frame(ps.menu.12@tax_table@.Data), 
               by = c('menu_taxa' = 'name')) %>% 
     distinct() %>% 
     select(superkingdom:species, mp_to_menu, menu_taxa) %>%
     column_to_rownames(var = 'menu_taxa') %>% 
     as.matrix()

ps.menu.id <- tax_glom(ps.menu.12, taxrank = 'mp_to_menu')
...
Think I need to filter out cases where the denominator of either FPR or FNR is NA in advance.
...
```{r}
protein.roc <- 
     protein.pos[, !(colnames(protein.pos) %in% drop.na)] %>% 
     apply(2, as.numeric)

menu.roc <- 
     menu.pos[, colnames(protein.roc)] %>% 
     apply(2, as.numeric)
```

```{r}
x <- prediction(predictions = protein.roc, labels = menu.roc)
y <- performance(x, 'tpr', 'fpr')
...
```{r}
dna.roc <- 
     dna.pos[, !(colnames(dna.pos) %in% drop.na)] %>% 
     apply(2, as.numeric)

menu.roc <- 
     menu.pos[, colnames(dna.roc)] %>% 
     apply(2, as.numeric)
```

```{r}
x <- prediction(predictions = dna.roc, labels = menu.roc)
y <- performance(x, 'tpr', 'fpr')
