...
# We'll aim for row lengths of 29, which will correspond to 
# 4 rows of plant taxa
# 1 row of animal taxa
# 1 row of other taxa
row_len <- 29
```
...
```{r}
common_names <- levels(melt$common_name)
...
sliced_vector <- slice_vector(common_names, row_len)
...
```{r}
for (r in sliced_vector){
     melt %>%
          filter(common_name %in% r)
          ggplot(aes(x = common_name, y = n, fill = prediction)) +
          geom_bar(position = 'fill', stat = 'identity') +
          scale_fill_manual(values = c('#309343','#8AC373', # TP, TN
                                       '#D7191C', '#FDAE61',# FP, FN
