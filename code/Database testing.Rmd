...
Note that metabarcoding is a bit sloppy because I'll want different reference days for plant and animal markers (because fewer samples amplified with trnL than with 12SV5). Sort this out when calculating TP, TN, etc.
...
# Merge
ps.mb.id <- merge_phyloseq(ps.mb.plant.id,
                           ps.mb.animal.id)
```
...
```{r}
name.sync %>% 
     filter(`taxon index` %in% as.numeric(fp)) %>% 
     select(mp_taxa, menu_taxa)
```
...
asvtab.mb <- asvtab.mb[row.names(asvtab.menu.12), ]
all(row.names(asvtab.mb) == row.names(asvtab.menu.12))
```

```{r}
# Label predictions (this may be best to do separately for plants, animals to include only those amplified samples)
dna.pos <- asvtab.mb > 0 # Already filtered, above
menu.pos <- asvtab.menu.12 > 0
