...
tax_table(ps.protein) <- 
     name.sync %>% 
     select(index = `taxon index`, mp_taxa) %>% 
...
ps.protein.id <- tax_glom(ps.protein, taxrank = 'index')
taxa_names(ps.protein.id) <- trimws(tax_table(ps.protein.id)[, 'index'])
```
...
tax_table(ps.menu.12) <- 
     name.sync %>% 
     select(index = `taxon index`, menu_taxa) %>% 
...
ps.menu.id <- tax_glom(ps.menu.12, taxrank = 'index')
taxa_names(ps.menu.id) <- trimws(tax_table(ps.menu.id)[, 'index'])
```
