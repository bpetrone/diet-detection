...
ggsave(here('results',
            'manuscript',
            '2',
            paste0(plotID, '_Plant metaproteomic ordination (Jaccard).pdf')),
       device = cairo_pdf,
       height = 4, width = 4)
```
...
ord.mb.plant <- 
     ps.mb.plant %>% 
     # Need to remove samples w/ 0 counts, error otherwise
     prune_samples(sample_sums(.) > 0, .) %>% 
     ordinate(.,
              method = 'PCoA',
              distance = 'jaccard',
...
samples <- intersect(sample_names(ps.protein.plant),
                     sample_names(ps.mb.plant.nonzero))
...
samples <- intersect(sample_names(ps.protein.animal),
...
```{r}
vegan::mantel(distance(prune_samples(samples, ps.mb.plant),
                       method = 'jaccard',
                       binary = TRUE),
              distance(prune_samples(samples, ps.protein.plant),
                       method = 'jaccard',
                       binary = TRUE))
```

```{r}
vegan::mantel(distance(
     prune_samples(
...
```{r}
pca.protein.plant <- 
     prcomp(ps.protein.plant.clr@otu_table@.Data,
            center = TRUE,
            scale = FALSE)
```
...
```{r}
dist.protein.plant <- 
     data.frame(pca.protein.plant$x) %>% 
     dist() %>% 
     as.matrix() %>% 
     data.frame() %>% 
...
```{r}
pca.protein.animal <- 
     ps.protein.animal.clr@otu_table@.Data %>% 
     prcomp(center = TRUE,
            scale = FALSE)
```
...
```{r}
dist.protein.animal <- 
     data.frame(pca.protein.animal$x) %>% 
     dist() %>% 
     as.matrix() %>% 
     data.frame() %>% 
     rownames_to_column(var = 'x') %>% 
...
```{r}
# Plot
ggplot(dist.mb.plant, aes(x = delta, 
                          y = distance,
                          group = delta,
                          by = delta)) +
     geom_boxplot()
...
```{r}
pca.mb.animal <- 
     prcomp(ps.mb.animal.clr@otu_table@.Data,
            center = TRUE,
            scale = FALSE)
```
...
```{r}
# Plot
ggplot(dist.mb.animal, aes(x = delta, 
                           y = distance,
                           group = delta,
                          by = delta)) +
     geom_boxplot()
