...
ps.menu <- 
     ps.menu %>% 
     prune_samples(sample_names(.) %in% c(sample_data(ps.mb.plant)$delta1,
...
```{r}
vegan::mantel(distance(prune_samples(samples, ps.mb.animal),
                       method = 'jaccard',
                       binary = TRUE),
              distance(prune_samples(samples, ps.protein.animal),
                       method = 'jaccard',
                       binary = TRUE))
```
...
```{r}
# Plot
ggplot(dist.protein.animal, aes(x = delta, 
                          y = distance,
                          group = delta,
                          by = delta)) +
     geom_boxplot()
