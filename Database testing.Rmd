...
```{r}
# Manual correction to match database (V.radiata, M. piperita)
...
Just do manually for now: write main table, trim these rows, and read back in
...
add.rows <-
     here('data', 'processed', 'miniseq', '20191206',
          '20200828_Internal taxonomy table nodes.csv') %>%
...
lowest.index <- max.col(!is.na(taxtab.menu), 'last')

# Make row-column pairs to pull and add to taxonomy
taxtab.menu$lowest.level <- taxtab.menu[cbind(seq_along(lowest.index),
                                            lowest.index)]
