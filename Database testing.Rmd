...
```{r}
ps.trnL <- subset_samples(ps.trnL, anneal_t == '63')
# Remove taxa that now have 0 counts (67 in all)
ps.trnL <- prune_taxa(taxa_sums(ps.trnL) != 0, ps.trnL)
ps.trnL
```

Remove sequence variants not identified to a food in current database
```{r}
ps.trnL.foods <- subset_taxa(ps.trnL, !is.na(superkingdom)) # Removes 32 taxa
```

What percentage of reads is this?

```{r}
# Pull taxonomic data
taxdf <- 
     tax_table(ps.trnL.foods) %>%
     as.data.frame() %>%
     rownames_to_column(var = 'asv')

# Join to sums
tax.sums <- 
     data.frame(sum = taxa_sums(ps.trnL)) %>%
     rownames_to_column(var = 'asv') %>%
     full_join(taxdf, by = 'asv')
```

```{r}
total <- sum(tax.sums$sum)

filter(tax.sums, !is.na(superkingdom)) %>%
     select(sum) %>%
     sum()/total
```
