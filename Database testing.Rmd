...
proteins.df$OS[proteins.df$OS == 'Oryza sativa subsp. japonica'] <- 'Oryza sativa'
proteins.df$OS[proteins.df$OS == 'Musa acuminata subsp. malaccensis'] <- 'Musa acuminata'

# Manual correction to match NCBI taxonomy (A. bisporus, A. platensis)

proteins.df$OS[proteins.df$OS == 'Agaricus bisporus var. burnettii (strain JB137-S8 / ATCC MYA-4627 / FGSC 10392)'] <- 'Agaricus bisporus'
proteins.df$OS[proteins.df$OS == 'Arthrospira platensis (strain NIES-39 / IAM M-135)'] <- 'Arthrospira platensis'
proteins.df$OS[proteins.df$OS == 'Arthrospira platensis C1'] <- 'Arthrospira platensis'
proteins.df$OS[proteins.df$OS == 'Laetiporus sulphureus 93-53'] <- 'Laetiporus sulphureus'
```
...
taxtab.protein <-
     taxa::taxonomy_table(taxids,
                          use_ranks = c('superkingdom', 'kingdom',
                                        'phylum', 'superorder', 'order', 
                                        'suborder', 'class', 'superfamily',
...
Think this is because some are internal nodes.
Figure this out and add their taxonomy too.
...
```
So yes, this seems to be what's happening.
Now quickly build the rows to add to the table
