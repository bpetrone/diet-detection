...
# saveRDS(ps.protein, here('data', 'processed', '20210212_ps.rds'))
```

## Metaproteomic phyloseq

```{r}
ps.protein <- readRDS(here('data', 'processed', '20210212_ps.rds'))
ps.protein
```
...
```{r}
ps.menu <- readRDS(here('data', 'processed', '20210215_ps_menu.rds'))
ps.menu
```
...
ggplot(counts.long, aes(x = dataset, y = value)) +
     geom_boxplot() +
     labs(y = 'Number of taxa', x = 'Source') +
     theme_bw()
```
...
day.before <- intersect(samdf.protein$delta1, samdf.menu$row)
```
