...
ps.protein <- readRDS(here('data', 'processed', 'phyloseq',
                           '20210408_ps_mp.rds')) # V2 analysis
                           # '20210727_ps_mp.rds')) # V3 analysis
ps.protein
```
...
What species are shared between metaproteomic, metabarcoding, and menu datasets (regardless of date?)
```{r}
...
taxtab.menu <- ps.menu@tax_table@.Data
```

```{r}
# Get only asvtab entries that pass the 5 PSM filter
dim(asvtab.protein)
taxa.protein <- 
     colnames(asvtab.protein)[apply(asvtab.protein, 
                                    2, 
                                    function(x){any(x >= 5)})]
```
...
```{r}
fp <- setdiff(taxa.protein, row.names(taxtab.menu))
head(fp)
length(fp)
```
...
Note: had a few genus-level entries here that I removed, because there was a finer species-level entry that captured them, and it would have been hard to synchronize shift to family or genus level otherwise.  These were Coffea, Citrus, Musa, Prunus, Pyrus, and Fragaria.
...
entries <- table(pred.summary$certain)
entries

names(entries) <- NULL
n.certain <- entries[2]
n.uncertain <- entries[1]
rm(entries)

rows.certain <- ceiling(n.certain/25)
rows.uncertain <- ceiling(n.uncertain/25)

if (rows.uncertain > 1) {
     pred.summary$facet <- c(cut(1:n.certain, 
                                 breaks = rows.certain, 
                                 labels = seq(rows.certain)),
                             cut(1:n.uncertain, 
                                 breaks = rows.uncertain))
} else {
     pred.summary$facet <- c(cut(1:n.certain, 
                                 breaks = rows.certain,
                                 labels = seq(rows.certain)),
...
pred.long <- left_join(pred.long, 
                       taxtab,
                       by = c('taxon' = 'family'))

# Recode NA kingdom as "Other"
pred.long$kingdom[is.na(pred.long$kingdom)] <- 'Other'
```
...
saveRDS(pred.long,
        here('data', 'processed', 'performance-metrics', 
             '20210817_Metaproteomic v. menu predictions by food family, 1-2d prior, >4 counts per taxon filter, v2 analysis.rds'))
```
...
pred.long %>%
     ...
     scale_fill_manual(values = c('#1A9641','#A6D96A', # TP, TN
                                  '#D7191C', '#FDAE61')) + # FP, FN
     expand_limits(x = -2, y = -15) +
     # Create a strip of white background under the fake axis
     geom_rect(ymin = -15, ymax = 0, 
               xmin = 0, xmax = n_distinct(pred.long$taxon), 
               fill = "white") + 
     # Fake axis layer, aligned below y = 0
     geom_text(aes(color = kingdom, label = taxon), 
               y = 0, 
               vjust = 1.1, hjust = 1,
               angle = 45) +
...
ggsave(here('results', 
            ...
            paste0(plotID, 
                   '_By-food family predictions compared to menu, >4 PSMs per taxon filter, >1 oz intake filter, v2 analysis, certain.pdf')),
       height = 10, width = 8)
...
# 2021-07-28.966: Analysis v3, >4 PSMs/taxon/sample filter
# 2021-08-16.349: Analysis v3, >4 PSMs/taxon/sample filter, family glom
```
...
ggsave(here('results', 
            'per-food performance',
            paste0(plotID, 
                   '_By-food family predictions compared to menu, >4 PSMs per taxon filter, >1 oz intake filter, v2 analysis, uncertain.pdf')),
       height = 2.5, width = 8)
```
