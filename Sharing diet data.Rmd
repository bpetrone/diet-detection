...
menus <-
     ...
     rename(menu_item_name = menu_item_name) %>% 
...
menus.13.unique <-
     here('..', 'dfc', 'data', 'processed', 'RealChoices',
          'WORKING_PC74,AK65,ML48_Coded menu v4 (quantity).csv') %>%
...
menus.13.unique <- 
     here('..', 'dfc', 'data', 'processed', 'RealChoices',
          'WORKING_JN55_Coded menu v3 (quantity).csv') %>%
     read_csv(col_types = cols(.default = 'f',
                               ingredient_list = 'c',
                               family = 'c', subfamily = 'c',
                               genus = 'c', species = 'c', subspecies = 'c',
...
menus.outside <-
        here('..', 'dfc', 'data', 'processed', 'RealChoices',
             'WORKING_Outside meals manual export v4 (quantity).csv') %>%
        read_csv(col_types = cols(.default = 'f',
                                  Date = 'c',
                                  ingredient_list = 'c',
                                  family = 'c', subfamily = 'c',
...
# Add location information to DFC menu
menus.13$location = 'DFC cafeteria'
```
...
menus.13 <- filter(menus.13, !(menu_item_name %in% exclude))
...
Ali requested 2 days of eating prior to the sample, except for PC74.  For this subject, share all available data.
...
menu.subset <- 
     ...
     select(-delta) %>% 
     left_join(menus.13) %>% 
     select(ID, date, meal, menu_item_name, `serving size`, 
...
diet.subset.pc74 <- 
     menus.13 %>%
     filter(ID == 'PC74') %>% 
     select(ID, date, location, meal, menu_item_name, `serving size`, 
            est_grams_consumed_total, `Item Name`, taxon_name, animal:fungus)   
            # Refine columns to only those Ali will need
```
...
write_csv(diet.subset.pc74,
          here('data', 'processed', '20210716_PC74 full menu.csv'))
```
